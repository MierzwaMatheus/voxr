...
def make_app(tmp_path, model_name="medium", model_loaded=True):
    """Helper: create a VoxrApp wired for unit testing.

    model_loaded=True → a dummy model object is injected (simulates loaded model).
    model_loaded=False → model is None (simulates missing model).
    """
    ...
    model_obj = MagicMock() if model_loaded else None
    app = VoxrApp(config=fake_config, model=model_obj)
...
class TestDoProcessModelCheck:
    def test_notifies_and_aborts_when_model_bin_missing(self, tmp_path, monkeypatch):
        ...
        app, model_dir = make_app(tmp_path, model_loaded=False)

        transcribe_called = []
        import voxr.transcription as trans_mod

        monkeypatch.setattr(
            trans_mod, "transcribe_session", lambda *a, **kw: transcribe_called.append(True)
        )
    ...
    def test_proceeds_when_model_bin_present(self, tmp_path, monkeypatch):
        ...
        app, model_dir = make_app(tmp_path, model_loaded=True)
        ...
        assert transcribe_called, (
            "transcribe_session deve ser chamado quando model.bin está presente"
        )
